...
{
    public class SalesCounterExportStrategy : IExportStrategy
    {
        private string _templatePath;
        private SalesCounterExportObject _itemsToExport;
        private const int rowsPerPage = 35;
        private const int headerRows = 6;
        private const int footerRows = 6;

        public SalesCounterExportStrategy(SalesCounterExportObject items)
        {
            this._templatePath = Directory.GetCurrentDirectory() + @"\Templates\SalesCounterTemplate.xls";
            ...
            try
            {
                ...
                if(book.Worksheets.Count > 0)
                {
                    ...
                    foreach (var item in this._itemsToExport.Items)
                    {
                        ...
                        invoiceCell.Value2 = item.SaleId != null ? item.InvoiceNumber : string.Format("({0})", item.MemoNumber);
                        ...
                        amountCell.Value2 = item.SaleId != null ? item.Amount.GetValue() :
                    ...
                    {
                        Excel.Range discountRange = sheet.get_Range("C28, H28");
                        discountRange.Value2 = "less: discount";

                        Excel.Range discountValRange = sheet.get_Range("D28, I28");
                        discountValRange.Value2 = this._itemsToExport.Discount;
                    }

                    if (this._itemsToExport.WitholdingTax > 0)
                    {
                        Excel.Range taxRange = sheet.get_Range("C29, H29");
                        taxRange.Value2 = "less: w/tax";

                        Excel.Range taxValRange = sheet.get_Range("D29, I29");
